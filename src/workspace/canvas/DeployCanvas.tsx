...
export default function DeployCanvas({ engine }: DeployCanvasProps) {
  const [copied, setCopied] = useState<string | null>(null);
  const [exportingAs, setExportingAs] = useState<string | null>(null);
  
  const { metrics, statusMessage, exportModel } = engine;
  ...
  const handleExport = (format: string) => {
    ...
    setTimeout(() => setExportingAs(null), 3500);
  };
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="rounded-lg p-4" style={{ background: 'var(--bg-panel)', border: '1px solid var(--border-panel)' }}>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="ml-auto badge badge-green">{engine.isConnected ? 'Ready' : 'Offline'}</span>
          </div>
          <div className="space-y-3">
            <div>
              ...
              <div className="flex gap-1">
                ...
                <button onClick={() => handleCopy('endpoint', apiEndpoint)} className="p-1.5 rounded transition-colors hover:bg-[var(--bg-elevated)]">
            ...
            <div>
              ...
              <div className="relative">
                <pre className="text-xs font-mono p-3 rounded overflow-x-auto leading-relaxed" style={{ background: 'var(--bg-surface)', color: 'var(--text-secondary)' }}>
                  {curlExample}
                </pre>
                <button onClick={() => handleCopy('curl', curlExample)} className="absolute top-2 right-2 p-1 rounded hover:bg-[var(--bg-elevated)]">
        ...
        <div className="rounded-lg p-4" style={{ background: 'var(--bg-panel)', border: '1px solid var(--border-panel)' }}>
          ...
          <div className="space-y-2">
            <button onClick={() => handleExport('gguf')} disabled={!isTrained || exportingAs !== null} className="w-full ctrl-input text-xs text-left hover:border-[var(--accent)] cursor-pointer disabled:opacity-50 flex items-center justify-between">
            ...
            <button onClick={() => handleExport('safetensors')} disabled={!isTrained || exportingAs !== null} className="w-full ctrl-input text-xs text-left hover:border-[var(--accent)] cursor-pointer disabled:opacity-50 flex items-center justify-between">
